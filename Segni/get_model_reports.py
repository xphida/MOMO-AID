# -*- coding: utf-8 -*-
"""
Created on Tue May  5 23:01:31 2020
...
def plot_confusion_matrix(cm,
    ...
    normalize:    If False, plot the raw numcmap = plt.pyplot.get_cmap()('Blues')bers
    ...
    plt.pyplot.imshow(cm, interpolation='nearest', cmap=cmap)
    plt.pyplot.title(title)
    plt.pyplot.colorbar()

    if target_names is not None:
        tick_marks = np.arange(len(target_names))
        plt.pyplot.xticks(tick_marks, target_names, rotation=45)
        plt.pyplot.yticks(tick_marks, target_names)
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        if normalize:
            plt.pyplot.text(j, i, "{:0.4f}".format(cm[i, j]),
                     horizontalalignment="center",
                     color="white" if cm[i, j] > thresh else "black")
        else:
            plt.pyplot.text(j, i, "{:,}".format(cm[i, j]),
                     horizontalalignment="center",
                     color="white" if cm[i, j] > thresh else "black")
    ...
    plt.pyplot.savefig('confusion_matrix.png')
